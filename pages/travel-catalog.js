...
export default function Home() {
  ...
  async function buyNft(pikblocktravel) {
    ...
    const price = ethers.utils.parseUnits(
      pikblocktravel.price.toString(),
      'ether'
    );
    const transaction = await contract.createMarketSale(
      pikblocktraveladdress,
      pikblocktravel.itemId,
      {
        value: price,
      }
    );
  ...
  if (loadingState === 'loaded' && !nfts.length)
    return (
      <h1 className="bg-pink-600 flex flex-col justify-center space-y-1 px-4 mt-4">
        <Link href="/travel">
          <a className="bg-pink-600 px-10 py-4 text-1xl flex justify-center rounded">
            PikBlock travel
          </a>
        </Link>
            ...
            {currentItems.map((nft, i) => (
              <div key={i} className="border shadow rounded-xl overflow-hidden">
                ...
                <div className="w-full p-4 bg-black">
                  ...
                  <button
                    className="w-full bg-pink-600 text-white font-bold py-2 px-12 rounded"
                    onClick={() => buyNft(nft)}
                  >
                    Buy
                  </button>
