...
export default function Home() {
  ...
  async function buyNft(pikblockextra) {
    ...
    const price = ethers.utils.parseUnits(
      pikblockextra.price.toString(),
      'ether'
    );
    const transaction = await contract.createMarketSale(
      pikblockextraddress,
      pikblockextra.itemId,
      {
        value: price,
      }
    );
  ...
  if (loadingState === 'loaded' && !nfts.length)
    return (
      <h1 className="bg-yellow-400 flex flex-col justify-center space-y-1 px-40 mt-4">
        <Link href="/extra">
          <a className="bg-yellow-400 px-10 py-4 text-1xl flex justify-center rounded">
            PikBlock extra
          </a>
        </Link>
            ...
            {currentItems.map((nft, i) => (
              <div key={i} className="border shadow rounded-xl overflow-hidden">
                ...
                <div className="p-4 bg-black">
                  ...
                  <button
                    className="w-full bg-yellow-400 text-white font-bold py-2 px-12 rounded"
                    onClick={() => buyNft(nft)}
                  >
                    Buy
                  </button>
