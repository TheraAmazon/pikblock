...
import {
  pikblockportraitaddress,
  pikblockportraitMarketaddress,
} from '../config';
...
export default function Home() {
  ...
  async function loadNFTs() {
    ...
    const tokenContract = new ethers.Contract(
      pikblockportraitaddress,
      PIKBLOCKPORTRAIT.abi,
      provider
    );
    const marketContract = new ethers.Contract(
      pikblockportraitMarketaddress,
      PIKBLOCKPORTRAITMarket.abi,
      provider
    );
  ...
  async function buyNft(pikblockportrait) {
    ...
    const price = ethers.utils.parseUnits(
      pikblockportrait.price.toString(),
      'ether'
    );
    const transaction = await contract.createMarketSale(
      pikblockportraitddress,
      pikblockportrait.itemId,
      {
        value: price,
      }
    );
  ...
  if (loadingState === 'loaded' && !nfts.length)
    return (
      <h1 className="bg-purple-600 flex flex-col justify-center space-y-1 px-4 mt-4">
        <Link href="/portrait">
          <a className="bg-purple-600 px-10 py-4 text-1xl flex justify-center rounded">
            PikBlock Portrait
          </a>
        </Link>
