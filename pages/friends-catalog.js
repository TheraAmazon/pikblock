...
export default function Home() {
  ...
  async function buyNft(pikblockfriends) {
    ...
    const transaction = await contract.createMarketSale(pikblockfriendsaddress, pikblockfriends.itemId, {
      value: price
    })
            ...
            currentItems.map((nft, i) => (
              <div key={i} className="border shadow rounded-xl overflow-hidden">
                ...
                <div className="w-full p-4 bg-black">
                  ...
                  <button className="btn w-full bg-blue-400 text-white font-bold py-2 px-12 rounded" onClick={() => buyNft(nft)}>Buy</button>
                </div>
              </div>
            ))
          }
