...
import {
  pikblockfamilyaddress, pikblockfamilyMarketaddress
} from '../config'
...
export default function Home() {
  ...
  async function loadNFTs() {    
    ...
    const tokenContract = new ethers.Contract(pikblockfamilyaddress, PIKBLOCKFAMILY.abi, provider)
    const marketContract = new ethers.Contract(pikblockfamilyMarketaddress, PIKBLOCKFAMILYMarket.abi, provider)
  ...
  async function buyNft(pikblockfamily) {
    ...
    const contract = new ethers.Contract(pikblockfamilyMarketaddress, PIKBLOCKFAMILYMarket.abi, signer)

    const price = ethers.utils.parseUnits(pikblockfamily.price.toString(), 'ether')
    const transaction = await contract.createMarketSale(pikblockfamilyaddress, pikblockfamily.itemId, {
      value: price
    })
    await transaction.wait()
    loadNFTs()
  }
  if (loadingState === 'loaded' && !nfts.length) return (<h1 className="bg-green-500 px-5 py-4 text-1xl">
    <Link href="/family">
            <a className="bg-green-500 px-10 py-4 text-1xl flex justify-center">
              PikBlock family
            </a>
          </Link>
            ...
            currentItems.map((nft, i) => (
              <div key={i} className="border shadow rounded-xl overflow-hidden">
                ...
                <div className="w-full p-4 bg-black">
                  ...
                  <button className="btn w-full bg-green-500 text-white font-bold py-2 px-12 rounded" onClick={() => buyNft(nft)}>Buy</button>
                </div>
              </div>
            ))
          }
