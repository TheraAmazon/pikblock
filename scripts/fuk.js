...
async function main() {
  ...
  //PIKBLOCK - LIFESTYLE
  const PIKBLOCKLIFESTYLEMarket = await hre.ethers.getContractFactory("PIKBLOCKLIFESTYLEMarket");
  const pikblocklifestyleMarket = await PIKBLOCKLIFESTYLEMarket.deploy();
  await pikblocklifestyleMarket.deployed();
  console.log("pikblocklifestyleMarket deployed to:", pikblocklifestyleMarket.address);

  const PIKBLOCKLIFESTYLE = await hre.ethers.getContractFactory("PIKBLOCKLIFESTYLE");
  const pikblocklifestyle = await PIKBLOCKLIFESTYLE.deploy(pikblocklifestyleMarket.address);
  await pikblocklifestyle.deployed();
  console.log("pikblocklifestyle deployed to:", pikblocklifestyle.address);

  //PIKBLOCK - PORTRAIT
  const PIKBLOCKPORTRAITMarket = await hre.ethers.getContractFactory("PIKBLOCKPORTRAITMarket");
  const pikblockportraitMarket = await PIKBLOCKPORTRAITMarket.deploy();
  await pikblockportraitMarket.deployed();
  console.log("pikblockportraitMarket deployed to:", pikblockportraitMarket.address);

  const PIKBLOCKPORTRAIT = await hre.ethers.getContractFactory("PIKBLOCKPORTRAIT");
  const pikblockportrait = await PIKBLOCKPORTRAIT.deploy(pikblockportraitMarket.address);
  await pikblockportrait.deployed();
  console.log("pikblockportrait deployed to:", pikblockportrait.address);

  //PIKBLOCK - TRAVEL
  const PIKBLOCKTRAVELMarket = await hre.ethers.getContractFactory("PIKBLOCKTRAVELMarket");
  const pikblocktravelMarket = await PIKBLOCKTRAVELMarket.deploy();
  await pikblocktravelMarket.deployed();
  console.log("pikblocktravelMarket deployed to:", pikblocktravelMarket.address);

  const PIKBLOCKTRAVEL = await hre.ethers.getContractFactory("PIKBLOCKTRAVEL");
  const pikblocktravel = await PIKBLOCKTRAVEL.deploy(pikblocktravelMarket.address);
  ...
  fs.writeFileSync('configx.js', JSON.parse(data))
